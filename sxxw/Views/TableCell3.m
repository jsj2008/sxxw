...
- (UIScrollView *) getSv:(NSMutableArray *)newsarray{
    ...
    for (News *news in newsarray) {
        ...
        NSString *temptitleimg  = news.titleimg;
        [Arr addObject:[@"http://weyida.com" stringByAppendingString:temptitleimg]];
...
-(void)AdImg:(NSArray*)arr{
     [sv setContentSize:CGSizeMake(320*[arr count], 190)];
    page.numberOfPages=[arr count];
     for ( int i=0; i<[arr count]; i++) {
         NSString *url=[arr objectAtIndex:i];
         UIImageView *img = [[UIImageView alloc] initWithFrame:CGRectMake(320*i, 0, 320, 190)];
         [img setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"defaultpic.gif"]];
         [sv addSubview:img];
     }
}
